...
var displayCurrentWeatherData = function (weatherData) {
  ...
  document.querySelector(
    "#wind"
  ).innerHTML = `${weatherData.current.wind_speed} mph`;
  document.querySelector(
    "#humidity"
  ).innerHTML = `${weatherData.current.humidity} g.kg^-1`;
  document.querySelector(
    "#uv-index"
  ).innerHTML = `${weatherData.current.uvi} %rh`;
};

var displayWeatherForecast = function (weatherData) {
  // check if data was pulled
  if (weatherData.length === 0) {
    document.querySelector(".tile-container").innerHTML = "No data found";
  }
  ...
  weatherData.daily.forEach(function (day) {
    ...
    if (day.weather[0]["main"].toLowerCase() === "rain") {
      thumbnail = "./assets/images/storm-day.svg";
    }
    if (day.weather[0]["main"].toLowerCase() === "clouds") {
      thumbnail = "./assets/images/cloudy-day.svg";
    }
    if (day.weather[0]["main"].toLowerCase() === "snow") {
      thumbnail = "./assets/images/snow.svg";
    }
    if (day.weather[0]["main"].toLowerCase() === "clear") {
      thumbnail = "./assets/images/sunny-day.svg";
    }
    var html = `
